...
//    let searchController = UISearchController()
    ...
    func setupSearchBar(){
        ...
        // 🍎 2) 서치(결과)컨트롤러의 사용 (복잡한 구현 가능) -> 글자마다 검색 가능 + 새로운 화면을 보여주는 것도 가능
...
//    // 1️⃣ 유저가 글자를 입력하는 순간마다 호출되는 메서드
...
//
//        // 네트워킹 시작
//        networkManger.fetchMusic(searchTerm: searchText) { result in
//            switch result {
//            case .success(let musicDatas):
//                self.musicArrays = musicDatas
//                DispatchQueue.main.async {
...
//
//
//    // 2️⃣ 검색(Search) 버튼을 눌렀을때 호출되는 메서드
...
//
//        // 네트워킹 시작
//        networkManger.fetchMusic(searchTerm: text) { result in
//            switch result {
//            case .success(let musicDatas):
//                self.musicArrays = musicDatas
//                DispatchQueue.main.async {
...
extension ViewController: UISearchResultsUpdating {
    ...
    func updateSearchResults(for searchController: UISearchController) {
        ...
        // 글자를 치는 순간에 다른 화면을 보여주고 싶다면 (컬렉션뷰 보여줌)
