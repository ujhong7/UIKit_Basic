...
class DetailViewController: UIViewController {
    ...
    // MARK: - SAVE버튼 or UPDATE버튼이 눌렸을때의 동작 ⭐️⭐️⭐️
    
    @objc func saveButtonTapped(){
        print(#function)
        // 1️⃣ 멤버가 없다면 (새로운 멤버를 추가하는 화면)
        if member == nil{
            ...
            //새로운 멤버 (구조체) 생성
            var newMember = Member(name: name, phone: phoneNumber, address: address)
        ...
        } // 2️⃣ 멤버가 있다면 (멤버의 내용을 업데이트 하기 위한 설정)
        else{
            // 이미지에 있는 것을 그대로 다시 멤버에 저장
