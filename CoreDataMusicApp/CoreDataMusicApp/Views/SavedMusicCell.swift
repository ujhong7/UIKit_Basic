...
//  CoreDataMusicApp
//
//  Created by yujaehong on 2023/08/04.
//
...
final class SavedMusicCell: UITableViewCell {
    ...
    // 델리게이트 대신 실행하고 싶은 클로저 저장
    // 뷰컨트롤러에 있는 클로저 저장할 예정 (셀을 전달)
    var saveButtonPressed: ((SavedMusicCell) -> ()) = { (sender) in }
    
    // 델리게이트 대신 실행하고 싶은 클로저 저장
    // 뷰컨트롤러에 있는 클로저 저장할 예정 (셀/텍스트 전달)
    ...
    @IBAction func saveButtonTapped(_ sender: UIButton) {
        // 뷰컨트롤러에서 전달받은 클로저를 실행 (내 자신 SavedMusicCell을 전달하면서) ⭐️
        saveButtonPressed(self)
    }
    
    
    @IBAction func updateButtonTapped(_ sender: UIButton) {
        // 뷰컨트롤러에서 전달받은 클로저를 실행 (내 자신 및 Text를 전달하면서) ⭐️
